...
def GoldsteinPrice(x: List[float]) -> float:
    ...
    return (fact1*fact2).reshape(-1,1)
...
def Hartmann(x: List[float]) -> float:
    ...
    return  (-(2.58 + outer) / 1.94).reshape(-1,1)
...
def Bohachevsky(x: List[float]) -> float:
    ...
    return (x1**2 +2*(x2**2)-0.3*np.cos(3*np.pi*x1)-0.4*np.cos(4*np.pi*x2)+0.7).reshape(-1,1)
